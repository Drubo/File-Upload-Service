...
var uploadServer = function uploadServer(host, port, options, fn){
	...
	var common = new common_file(this.options);
	...
	var server = http.createServer(function(request, response) {
		if (request.url == '/upload' && request.method.toLowerCase() == 'post') {
			if (common.originIsAllowed(request.headers.origin)) {
			  // CORS Access
			  if(request.headers.origin.substr(7) in origins){
				  response.writeHead(200, {'Access-Control-Allow-Origin': '*'});
			  }
			  console.log('Multipart/FormData upload is in Progress');
console.log(request.headers);
			  ...
			  form.addListener('file', function(name, file) {
				  ...
				  common.addJob(uploadFile); //Adds Job to BeansTalk
			  });
			  
			  form.addListener('error', function(err){
				 console.log(err); 
				 response.write("Error in Upload. Please try again later...");
				 response.end();
			  });
			  ...
			  form.parse(request, function(err, fields, files) {
				...
				console.log((new Date()) + ' Connection from origin ' + request.headers.origin + ' rejected.');
				response.write('You are not allowed to upload in this site.');
				response.end();
			}
		}else if(request.url == '/upload' && request.method.toLowerCase() == 'options') {
			if (common.originIsAllowed(request.headers.origin)) {
				// CORS Access
				if(request.headers.origin.substr(7) in origins){
					response.writeHead(200, {'Access-Control-Allow-Origin': '*'});
		...
		}else if(request.url=='/js/init.js'){
			var data = fs.readFileSync('js/init.js', 'utf8');
			data = data.replace(/maxFileSize/g,options.maxFileSize);
...
uploadServer.prototype.serveXhrPollingFormData = function(){
	...
	data = data.replace(/hostFromServer/g,this.host+':'+this.port);
	data = data.replace(/typesFromServer/g,'"'+this.options.allowed_types+'"');
...
uploadServer.prototype.serveIFrame = function(){
	...
	data = data.replace(/hostFromServer/g,this.host+':'+this.port);
	data = data.replace(/typesFromServer/g,'"'+this.options.allowed_types+'"');
	data = data.replace(/maxFileSize/g,this.options.maxFileSize);
